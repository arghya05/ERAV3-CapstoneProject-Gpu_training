...
```bash
# For local testing (Mac/PC with limited RAM):
...
## 🖥️ GPU Training (Recommended)

**If you have access to a GPU (much faster):**

### GPU Advantages:
- **10-20x faster training** (3-5 minutes vs 30-60 minutes)
- **Larger batch sizes** for better training
- **Higher precision** with FP16 support
- **More training steps** in less time

### GPU Requirements:
- **GPU Memory**: 16GB+ VRAM recommended (24GB+ ideal)
- **CUDA Support**: NVIDIA GPU with CUDA 11.8+
- **Cloud Options**: Google Colab Pro, AWS, RunPod, etc.

### Running on GPU:
```bash
# Same setup as above, but run the GPU version:
python train_llama3_gpu.py
```

### GPU vs Local Training Comparison:
| Feature | Local (Mac/PC) | GPU Training |
|---------|---------------|--------------|
| **Time** | 30-60 minutes | 3-5 minutes |
| **Batch Size** | 1 | 4-8 |
| **Max Steps** | 20-100 | 500+ |
| **Memory** | 8GB RAM | 16GB+ VRAM |
| **Cost** | Free | $0.50-2.00 |
